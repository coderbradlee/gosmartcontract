...
func GetMnemonic(password string) (string, error) {
	entropy, _ := bip39.NewEntropy(128)
...
func GetPrivateKeyByPath(mnemonic string, coinType string, account, change, index int) (*hdkeychain.ExtendedKey, error){
	mnemonicSeed, err := importMnemonic(mnemonic)
	if err != nil {
		return nil, err
	}
	if len(mnemonicSeed) < 2 {
		return nil, errors.New("The mnemonicSeed byte len is two low!!")
	}
	masterKeyStr, err := generateMasterkey(mnemonicSeed)
	if err != nil {
		return nil, err
	}
	extKey,err:=getChildByPath(masterKeyStr,coinType,account, change, index)
	if err != nil {
		return nil, err
	}
	// ECPrivKey() (*btcec.PrivateKey, error)
	// p,err:=extKey.ECPrivKey()
	// priKey:=p.ToECDSA()
	// if err != nil {
	// 	return nil, err
	// }

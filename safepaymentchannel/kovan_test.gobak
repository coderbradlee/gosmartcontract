...
func TestDeploy(t *testing.T) {
	...
	c := NewConnecterWithDeploy(selfhostkovan, ownerAuth)

	fmt.Println("Contract address is: ", c.contractAddress.String())

}
func TestAccounts(t *testing.T) {
	keybyte:=[]byte(userKeystore1)
	key, err := keystore.DecryptKey(keybyte, userPassphrase1)
	if err != nil {
		fmt.Printf("Error decrypting key: %v\n", err)
	}
	// fmt.Printf("%x", key.PrivateKey.D.Bytes())
	privKey:=fmt.Sprintf("%x", key.PrivateKey.D.Bytes())
	...
	for i:=0;i<50;i++{
		...
		fmt.Printf("The %d ETH privateKeyBytes is %s\n", i,private_str)
		{
			...
			c := NewConnecter(selfhostkovan, contractAddresskovan)

			c.Send(privKey,addressStr,big.NewInt(20000000000000000))
...
func TestBuy(t *testing.T) {
	// s := NewConnecter(selfhostkovan, contractAddresskovan)
...
func TestGet(t *testing.T) {
	...
	fmt.Println("########################################################")
	{
		...
		ret1, ret2, ret3, ret4, ret5, ret6,err := s.lottery.GetPlayerInfoByAddress(nil, common.HexToAddress(userAddress1))
		if err != nil {
			fmt.Println(err)
			// return
		}
	...
	fmt.Println("########################################################")
	{
		ret1, ret2, ret3, ret4, ret5, ret6, err := s.lottery.GetPlayerInfoByAddress(nil, common.HexToAddress(userAddress2))
		if err != nil {
			fmt.Println(err)
			// return
		}
